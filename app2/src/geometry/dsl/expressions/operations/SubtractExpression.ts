// Subtract expression for computing the difference of two numeric values
...
/**
 * Expression that computes the difference of two numeric values (a - b).
 * The result is stored as a point where x = difference and y = 0.
 * Use `.value` to reference the computed difference.
 */
export class SubtractExpression<TConfig> implements GeometryExpression<TConfig, "point"> {
  readonly id: string;
  ...
  /**
   * Reference to the computed difference value.
   ...
   * @returns A Step that computes the difference when executed
   */

// Multiply expression for computing the product of two numeric values
...
/**
 * Expression that computes the product of two numeric values (a * b).
 * The result is stored as a point where x = product and y = 0.
 * Use `.value` to reference the computed product.
 */
export class MultiplyExpression<TConfig> implements GeometryExpression<TConfig, "point"> {
  readonly id: string;
  ...
  /**
   * Reference to the computed product value.
   ...
   * @returns A Step that computes the product when executed
   */

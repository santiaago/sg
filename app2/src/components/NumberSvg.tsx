import { useEffect, useRef, useMemo, forwardRef } from "react";
import type { Ref } from "react";
import type { SvgConfig } from "../config/svgConfig";
import type { GeometryStore } from "../react-store";
import { rect, clearGeometryFromSvg } from "../svgElements";
import { setupSvg } from "../svg";
import { DefaultGeometryRenderer } from "../geometry/dsl/renderers/DefaultRenderer";
import { executeSteps } from "../geometry/stepExecution";
import { computeNumberConfig, type NumberConfig } from "../geometry/numbers/config";
import { useThemeAwareSteps } from "../hooks/useThemeAwareSteps";
import { buildStepMaps, pick } from "../svg";
import { darkTheme } from "../themes";
import type { Theme } from "../themes";
import type { Step } from "../types/geometry";

// Number registry mapping number to its module and metadata
export const NUMBERS = {
  1: { label: "1 - Dot", steps: 7 },
  2: { label: "2 - Two Circles", steps: 8 },
  3: { label: "3 - Three Circles", steps: 9 },
  4: { label: "4 - Four Circles", steps: 10 },
} as const;

export type NumberId = keyof typeof NUMBERS;

// Props for the NumberSvg component.
export interface NumberSvgProps {
  // Store for managing SVG elements and tooltips
  store: GeometryStore;

  // Stroke width for large elements (dots)
  dotStrokeWidth?: number;

  // SVG configuration (dimensions, classes)
  svgConfig: SvgConfig;

  // Key to trigger restart (e.g., when resetting the construction)
  restartTrigger?: number;

  // Number of steps to execute (0 = none, 1 = first step, N = N steps)
  currentStep?: number;

  // Theme for SVG rendering (light or dark)
  theme?: Theme;

  // Which number to render (1-4)
  number: NumberId;

  // Pre-built steps for this number
  steps: Step<NumberConfig>[];
}

/**
 * NumberSvg component - Renders the SVG canvas for a specific number's geometry construction.
 * This is the SVG-only version without player controls, parameterized by number.
 */
export const NumberSvg = forwardRef(function NumberSvg(
  {
    store,
    dotStrokeWidth = 2.0,
    svgConfig,
    restartTrigger = 0,
    currentStep = 0,
    theme = darkTheme,
    number,
    steps: allSteps,
  }: NumberSvgProps,
  ref: Ref<SVGSVGElement | null>,
): React.JSX.Element {
  const svgRef = useRef<SVGSVGElement>(null);

  // Use hook to track step and theme changes for clearing logic
  const { shouldClear } = useThemeAwareSteps({
    currentStep,
    restartTrigger,
    theme,
  });

  // Forward the ref to the SVG element
  useEffect(() => {
    if (!ref) return;
    if (typeof ref === "function") {
      ref(svgRef.current);
    } else {
      ref.current = svgRef.current;
    }
  }, [ref]);

  // Memoize the number configuration (derived from SVG dimensions)
  const numberConfig = useMemo<NumberConfig>(() => {
    return computeNumberConfig(svgConfig.width, svgConfig.height);
  }, [svgConfig.width, svgConfig.height]);

  // Memoize renderer to avoid recreating on every render
  const renderer = useMemo(() => new DefaultGeometryRenderer(""), []);

  // Effect 1: SVG container setup - ONLY when dimensions or theme change
  useEffect(() => {
    if (!svgRef.current) return;
    const svg = svgRef.current;

    // Clear everything and setup SVG container
    setupSvg(svg, svgConfig);

    // Draw the background rectangle using the theme color
    rect(svg, svgConfig.width, svgConfig.height, theme);
  }, [svgConfig.width, svgConfig.height, svgConfig.viewBox, theme]);

  // Effect 2: DSL construction and step execution - ONLY when step, restart, config, or theme changes
  useEffect(() => {
    if (!svgRef.current) return;

    const svg = svgRef.current;

    // Clear geometry and store when going backwards, restarting, or theme changes
    if (shouldClear) {
      clearGeometryFromSvg(svg);
      store.clear();
    }

    // If no steps to draw, exit
    if (currentStep <= 0) return;

    try {
      // Set current step ID for highlighting
      const currentStepId =
        currentStep > 0 && currentStep < allSteps.length ? allSteps[currentStep - 1]?.id : "";
      renderer.setCurrentStepId(currentStepId);

      // Build step maps for dependency tracking and parameter values
      const { stepDependencies, stepForOutput } = buildStepMaps(allSteps, currentStep);

      // Execute steps up to currentStep
      const allValues = executeSteps(allSteps, currentStep, { svg, store, theme }, numberConfig);

      // Build dependency map and step maps for parameter values
      if (currentStep > 0) {
        for (const [id, _] of allValues) {
          const deps = stepDependencies.get(id) ?? [];
          const step = stepForOutput.get(id);
          const paramValues = step?.parameters ? pick(numberConfig, step.parameters) : {};
          const stepId = step?.id ?? "";

          // Only update store for visual geometry
          if (step?.isVisual !== false) {
            store.update(id, {
              dependsOn: deps,
              stepId,
              parameterValues: paramValues,
            });
          }
        }
      }
    } catch (error) {
      console.error(
        "NumberSvg construction failed for number",
        number,
        "at step",
        currentStep,
        ":",
        error,
      );
    }
  }, [
    currentStep,
    restartTrigger,
    svgConfig,
    dotStrokeWidth,
    theme,
    shouldClear,
    numberConfig,
    allSteps,
  ]);

  return (
    <svg
      ref={svgRef}
      className={`${svgConfig.svgClass} block`}
      data-testid={`number-svg-${number}`}
    />
  );
});
